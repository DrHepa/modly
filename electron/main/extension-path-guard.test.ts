...
import { assertSafeExtensionId, buildExtensionBackupPath, resolveExtensionPathWithinRoot, resolvePathWithinRoot } from './extension-path-guard.ts'

test('assertSafeExtensionId accepts conservative extension ids', () => {
...
test('assertSafeExtensionId rejects empty, traversal and absolute ids', () => {
...
test('assertSafeExtensionId rejects uppercase and unsupported characters', () => {
  assert.throws(() => assertSafeExtensionId('Mesh-Process'), /must match/i)
  assert.throws(() => assertSafeExtensionId('bad id'), /must match/i)
  assert.throws(() => assertSafeExtensionId('bad:id'), /must match/i)
})

test('resolveExtensionPathWithinRoot confines paths to root', () => {
...
test('resolvePathWithinRoot rejects canonical escapes', () => {
  const root = path.join('/tmp', 'extensions-root')
...
test('buildExtensionBackupPath stays within root and rejects unsafe ids', () => {
  const root = path.join('/tmp', 'extensions-root')
